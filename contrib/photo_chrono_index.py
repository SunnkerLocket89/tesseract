...
def main():
    ...
    # Build full page list (Chronology_Working)
    chrono_rows = []
    global_idx = 0
    ...
    chrono["is_slate_candidate"] = chrono.apply(
        lambda r: (r["batch"], r["page_number"]) in slate_key, axis=1
    )

    # OCR slate candidates only
    ocr_out = []
    for _, r in slates.iterrows():
        ...
        # Text-layer for OCR PDFs, otherwise image OCR
    ...
    # Assign Run_ID by slate order (per batch)
    chrono["run_id"] = 0
    ...
    # Join slate OCR onto slate pages
    chrono = chrono.merge(
        slate_ocr[
            [
                "batch",
    ...
    # One row per (batch, run_id): pick first slate page per run
